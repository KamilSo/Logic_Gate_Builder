...
class Connection:
    ...
    def delete_line(self, e):  # Deletes the line and hitbox. Also updates all attributes and variables the line might
        ...
        try:  # deletes the oval the line is connected to from the main bulb class instance it is dragged from
            ...
            self.oval = None
        try:  # sets the counter of the main bulb output instance to 0 if the line does connect to the main bulb output
            self.set_counter(0)
        except:
            pass
    ...
    def check_pos(self):  # checks if the line is in a valid position
        ...
        if not self.snap_distance:  # if the line didn't snap to a valid oval the line is invalid
            self.delete_line(None)
            return False
        if self.get_counter() == 1 and self.root.gettags(self.oval)[0] == 'OUTPUT_BULB_GATE':  # checks if there is
    ...
    def set_counter(self, counter):  # changes the counter attribute in the main bulb output class
        for instance in Output_instances:
            ...
            if int(self.start_x) == int(center_coords[0]) and int(self.start_y) == int(center_coords[1]) and instance.get_counter() == 0:
                instance.set_counter(counter)
                self.output_instance = instance
                break
            elif int(self.end_x) == int(center_coords[0]) and int(self.end_y) == int(center_coords[1]) and instance.get_counter() == 0:
                instance.set_counter(counter)
                self.output_instance = instance
                break
...
def Initialize():
    ...
    Button_quit = Button(FuncBar_frame, text='X', command=window.quit, height=1, width=6, bd=0, bg=BUTTON_COLOUR, font='Helvetica')
    Button_quit.pack(side='right')

    Button_minmise = Button(FuncBar_frame, text='—', command=minimize_window, height=1, width=6, bd=0, bg=BUTTON_COLOUR, font='Helvetica')
    Button_minmise.pack(side = 'right')

    Title = Label(FuncBar_frame, text='Logic Gate Sim', height=1, width=14, bd=0, bg='#141414', font='Helvetica', fg = 'white')
    ...
    for index, name in enumerate(Gate_names):  # places all the gate name under the gate images
        var_name = name + 'label'
        globals()[var_name] = Label(window, text='yes', bg=BG_COLOUR, fg='white', font=('Verdana', 15))
